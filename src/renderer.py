...
def _text_html(text: TextBlock, references) -> str:
    ...
    content = _inline_text(text.text, references)
    return f"<p class=\"mark-text\"{style}>{content}</p>"
...
def _multi_image_html(images, figure_number: int) -> str:
    ...
    if not explicit:
        ...
        columns = " ".join(
            escape(width, quote=True) if width else
            f"minmax(0, calc((100% - ({total}) - {10 * (len(images) - 1)}px) / {remaining}))"
            for width in widths
        )
    ...
    html = [
        f'<figure class="article-image multi-image"{identifier}>',
        f'<div class="image-row" style="{row_style}">',
        "\n".join(image_html),
        "</div>",
    ]
    if images[0].caption:
...
def _list_html(list_block: ListBlock, environment_counters, references, figure_counter, depth: int) -> str:
    ...
    return (
        f'<{tag} class="mark-list"{type_attr} style="--list-color: {color}; --list-color-dark: {dark_color};">\n'
        + "\n".join(items)
        + f"\n</{tag}>"
    )
...
def _section_html(section: Section, number: int, environment_counters, references, figure_counter) -> str:
    ...
    html = [
        f'<section class="article-section" id="{identifier}" style="--section-color: {color}; --section-color-dark: {dark_color};">',
